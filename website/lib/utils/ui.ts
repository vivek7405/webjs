 ...
 *
 * The scaffold's convention, followed here so the site matches what
 * `webjs create` generates. A design system has two kinds of helper. A
 * repeated PRIMITIVE with variants (button, badge) is a CLASS helper
 * returning a class string; those live in lib/design.ts on this site, because
 * components/ui/ is a generated mirror here rather than owned files. A
 * repeated markup CHUNK (a page header, the closing CTA, the footer, the docs
 * shell) is an `html`-fragment helper, and belongs in this file.
 *
 * These render at SSR and disappear. Nothing here registers a custom element:
 * a fragment never reaches the browser, so making one an element would ship
 * JavaScript to register a tag that does nothing.
 */
    ...
    <footer class="mt-24 border-t border-border py-16 px-6 bg-bg-subtle/30">
      <div class="max-w-6xl mx-auto">
        <nav class="grid grid-cols-2 md:grid-cols-5 gap-8 mb-12" aria-label="Footer">
          ...
          <div class="flex flex-col gap-3">
            <a href="/" aria-label="WebJs home" class="no-underline text-fg inline-flex w-fit transition-opacity duration-fast hover:opacity-80">${brandLockup('ftr', { height: 22 })}</a>
