...
export const COMPONENT_API: Record<string, ComponentApi> = {
  ...
  'dropdown-menu': {
    ...
    subcomponents: [
      ...
      { name: '<ui-dropdown-menu-item>', description: 'Clickable row. variant="default | destructive", inset boolean. type="checkbox | radio" makes it a checkable item (role="menuitemcheckbox" / "menuitemradio" with aria-checked).' },
    ...
    props: [
      ...
      { name: 'type', type: '"checkbox" | "radio"', description: 'On <ui-dropdown-menu-item>. Makes it a checkable item: role="menuitemcheckbox" / "menuitemradio" plus aria-checked, with an indicator in the left gutter. Omit for a plain item.' },
      { name: 'checked', type: 'boolean (attribute, reflected)', default: 'false', description: 'On a checkable <ui-dropdown-menu-item>. Radio items keep exactly one checked per enclosing <ui-dropdown-menu-group>.' },
      { name: 'value', type: 'string (attribute, reflected)', description: 'On <ui-dropdown-menu-item>. Identifier reported in the ui-item-select detail; identifies a radio item within its group.' },
    ],
    events: [
      { name: 'ui-open-change', detail: '{ open: boolean }' },
      { name: 'ui-item-select', detail: '{ value: string, item: Element, type: string, checked: boolean }' },
    ],
  },
  ...
  'toggle-group': {
    ...
    props: [
      ...
      { name: 'disabled', type: 'boolean (attribute, reflected)', default: 'false', description: 'On <ui-toggle-group-item>. Refuses click and Enter / Space, reports aria-disabled, and is skipped by Arrow / Home / End so it never holds the group\u2019s single tab stop.' },
    ],
  ...
  sonner: {
    ...
    props: [
      ...
      },
      {
        ...
        description: 'On any toast call. Renders an action button that runs onClick then dismisses the toast.',
      },
      {
        name: 'cancel',
        type: '{ label: string, onClick: () => void }',
        description: 'On any toast call. Renders a secondary cancel button that runs onClick then dismisses. Every toast also ships a labelled close button, so a toast is always dismissable by hand.',
      },
    ],
  },
