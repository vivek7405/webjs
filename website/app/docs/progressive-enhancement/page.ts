...
export default function NewPost({ actionData }: {
  ...
  \`;
    ...
    <p>
      <strong>A per-button action is self-sufficient.</strong> The renderer supplies the submission attributes at the level where the action is bound, so a <code>formaction=\${action}</code> button gains <code>formmethod="post"</code> and <code>formenctype</code> ON THE BUTTON, alongside the identity riding its own <code>name</code>/<code>value</code> pair. A submitter's <code>formmethod</code> overrides the form's <code>method</code> per HTML, so the button posts correctly inside a bound form, an unbound form, a form with no method, or a <code>method="get"</code> form. That is what makes a submitter inside a component safe: the component renders its own template in a separate pass with no view of the host page, and it no longer needs one. Bind the enclosing form when the FORM itself should run an action on a plain submit.
    </p>
