...
export default function GlobalError({ error }: { error: Error }) {
  ...
  renderError(error: Error) {
    ...
    <p>The component boundary above also covers <code>watch(signal)</code> and <code>until()</code>, which commit outside the update cycle, so a throw from either reaches <code>renderError()</code> rather than the window. It reaches the component whose <em>template</em> holds the binding, which is not always the element the binding sits inside: a <code>watch()</code> written between a child component's tags belongs to the parent that wrote it. <code>asyncAppend</code> / <code>asyncReplace</code> are covered the same way: a chunk's own commit throw, and a <code>watch()</code> or <code>until()</code> nested inside a chunk, both reach the owning component's <code>renderError()</code>, and the stream then stops, since the boundary is about to render an error state and appending into a region it may have replaced is not a recovery. A throw from your own iterable is the exception: that is a generator failing rather than a render, so it is still logged to the console and your iterable is expected to handle it. It ends the stream too.</p>
