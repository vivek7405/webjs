...
class Report extends WebComponent {
  ...
  // customize the error UI.
    ...
    <p><strong>Telling whether it is working.</strong> A miss looks exactly like a hit from the outside, so development surfaces it two ways. Every dev page response carries an <code>X-Webjs-Seed</code> header (<code>collected=&lt;m&gt;, emitted=&lt;n&gt;</code> on a normal render, plus <code>off</code>, <code>html-cache</code>, and <code>... streamed</code> variants), folded into the access log as a <code>seed</code> field. And the browser logs one warning per page view when a hydration call missed, naming the cause: the page streams, the page carried no seeds at all, or it carried seeds but not for these calls (usually an argument the server render never used, such as one computed from browser-only state). Silence means every call hit. Nothing of this reaches production. See <a href="/docs/configuration">Configuration</a>.</p>
