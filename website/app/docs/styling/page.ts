...
.wordmark  { background: currentColor; }             /* the painted descendant */</code-block>
    ...
    <p>Read-only markup can be a fragment or a display-only component, and who renders it decides the cost. Under a page the two are equal and both free: the page is inert, so the fragment runs at SSR and is never fetched, and a component doing no client work is elided. Under an interactive island <em>both</em> ship, because an island's imports are fetched either way, so a fragment is not free there. What the component adds is an element class plus its registration, an upgrade per instance, and the display-only components it renders, which stop being elided along with it. That makes the fragment the smaller and steadier choice near an island rather than the free one.</p>
