...
.wordmark  { background: currentColor; }             /* the painted descendant */</code-block>
    ...
    <p>Read-only markup can be a fragment or a display-only component, and structure decides it, not bytes. A component is a tag in the DOM, so it adds a wrapper node. Reach for the fragment exactly where that node cannot exist: inside a <code>&lt;table&gt;</code>, where the HTML parser foster-parents an unknown element out of the table entirely; inside <code>&lt;select&gt;</code>, <code>&lt;ul&gt;</code>, or <code>&lt;dl&gt;</code>, where the wrapper breaks <code>ul &gt; li</code>, <code>:nth-child</code>, and the control's own semantics; as a direct grid or flex child, where the wrapper becomes the laid-out item; and where the output is an HTML string rather than DOM, such as a <code>&lt;webjs-stream&gt;</code> payload. Prefer the component everywhere else, since it gets a tag you can target and can grow behaviour later.</p>
