...
export default function RootLayout({ children }: { children: unknown }) {
  ...
  \`;
    ...
    <p><strong>A fixed header plus a modal that locks scroll.</strong> Locking page scroll hides the scrollbar, and a classic scrollbar takes real layout width, so hiding it widens the viewport. Padding the body compensates in-flow content but does nothing for a fixed header, which lays out against the initial containing block rather than the body's padding box, so it slides right by half the scrollbar width. <code>&lt;ui-dialog&gt;</code> and <code>&lt;ui-alert-dialog&gt;</code> handle this: the lock reserves the scrollbar gutter so the viewport width never changes, leaving it alone if your page already declared its own. Chromium and Firefox honour that and nothing moves. WebKit ignores <code>scrollbar-gutter</code>, so there the lock measures how much the viewport widened, pads <code>&lt;html&gt;</code> by it to hold in-flow content still, and publishes the amount as <code>--wj-scrollbar-compensation</code>, which a fixed element opts into with <code>padding-right: var(--wj-scrollbar-compensation, 0px)</code>. The property is set only while a lock is active and the viewport actually widened, so the <code>0px</code> fallback covers every other moment. Put that declaration on whichever element is off-centre: if your fixed wrapper paints nothing and a child carries the background, pad the inner centring container instead, or the painted chrome ends short of the edge.</p>
