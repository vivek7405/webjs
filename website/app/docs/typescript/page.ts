...
if (result.success) {
  ...
  console.log(result.data.slug);
    ...
    <p>Second case, and it is NOT narrowed: a value headed for an <code>html</code> template hole. A hole renders a string, a number, a <code>TemplateResult</code>, an array of those, a directive result, or nothing, so a helper like <code>lede(content: unknown)</code> is correctly typed and <code>TemplateResult</code> alone would be too narrow.</p>
    <p>Everywhere else it is a missing type, not a safe one: <code>unknown</code> surviving into a return type, a component prop, or an action signature is the shape to fix. <code>any</code> gets no carve-out at all, since it does not defer checking, it disables it.</p>
