...
if (result.success) {
  ...
  console.log(result.data.slug);
    ...
    <p>Where <code>unknown</code> IS right: a payload nothing has vouched for yet, narrowed on the very next line. A <code>route.ts</code> handler's <code>await req.json()</code>, an action's <code>export const validate</code>, a <code>catch</code> binding (already <code>unknown</code> under <code>strict</code>), and a value headed for an <code>html</code> template hole (a hole renders a string, a number, a <code>TemplateResult</code>, or an array of those, so <code>TemplateResult</code> alone would be too narrow). The test is what the next line does. <code>unknown</code> that survives into a return type, a component prop, or an action signature is a missing type, not a safe one. <code>any</code> gets no such carve-out: it does not defer checking, it disables it.</p>
