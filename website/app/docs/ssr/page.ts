...
async function loadExpensiveItems() {
  ...
  return html\`&lt;ul&gt;\${items.map(i =&gt; html\`&lt;li&gt;\${i.name}&lt;/li&gt;\`)}&lt;/ul&gt;\`;
    ...
    <p><strong>One exception to the table above: a function under <code>action=</code> or <code>formaction=</code> is never serialized.</strong> Stringifying a function writes its source into the HTML, and during SSR an imported <code>'use server'</code> action is the real function, so that source is the action's whole body. An unquoted <code>action=\${fn}</code> on a <code>&lt;form&gt;</code> is the one supported binding: the renderer resolves the action's identity, drops the attribute, and emits a hidden field (see <a href="/docs/server-actions">Server Actions</a>). Every other shape under those two names throws instead, including the boolean row and the native <code>.prop</code> row (on a <code>&lt;form&gt;</code>, or <code>.formAction</code> on a button or input, where the property reflects). Every other attribute behaves exactly as the table says, and a string-valued action is unchanged. See <a href="/docs/troubleshooting">Troubleshooting</a>.</p>
