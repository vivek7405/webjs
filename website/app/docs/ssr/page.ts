...
async function loadExpensiveItems() {
  ...
  return html\`&lt;ul&gt;\${items.map(i =&gt; html\`&lt;li&gt;\${i.name}&lt;/li&gt;\`)}&lt;/ul&gt;\`;
    ...
    <p><strong>One exception to the table above: a function under <code>action=</code> or <code>formaction=</code> is never serialized.</strong> Stringifying a function writes its source into the HTML, and during SSR an imported <code>'use server'</code> action is the real function, so that source is the action's whole body. An unquoted <code>action=\${fn}</code> on a <code>&lt;form&gt;</code>, or an unquoted <code>formaction=\${fn}</code> on a <code>&lt;button&gt;</code> inside a bound form, is supported: the renderer resolves the action's identity and emits the reserved identity field (see <a href="/docs/server-actions">Server Actions</a>). Unsupported submitter types and conflicting submitter attributes throw instead. Every other shape under those two names throws, including the boolean row and the native <code>.prop</code> row (on a <code>&lt;form&gt;</code>, or <code>.formAction</code> on a button or input, where the property reflects). Every other attribute behaves exactly as the table says, and a string-valued action is unchanged. See <a href="/docs/troubleshooting">Troubleshooting</a>.</p>
