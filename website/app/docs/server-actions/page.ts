...
export default function NewPost({ actionData }: {
  ...
  \`;
    ...
    <p>Submit buttons can also bind per-button server actions via <code>formaction=\${action}</code> on a <code>&lt;button&gt;</code> or <code>&lt;input type="submit"&gt;</code> inside a bound form. Without JavaScript, the clicked submitter button posts its identity field and takes precedence over the form's default action. Submitters cannot carry their own <code>name</code> attribute, use <code>&lt;input type="image"&gt;</code>, or declare <code>formmethod="get"</code> / <code>formenctype="text/plain"</code>.</p>
