...
export default function NewPost({ actionData }: {
  ...
  \`;
    ...
    <p>Submit buttons can also bind per-button server actions via <code>formaction=\${action}</code> on a <code>&lt;button&gt;</code> or <code>&lt;input type="submit"&gt;</code> inside a bound form. Without JavaScript, the clicked submitter posts its identity field and takes precedence over the form's default action. Only actual submit controls are supported. Submitters cannot carry their own <code>name</code>, <code>value</code>, or <code>form</code> attribute, a static <code>formaction</code>, use <code>&lt;input type="image"&gt;</code>, or declare <code>formmethod="get"</code> / <code>formenctype="text/plain"</code>.</p>
