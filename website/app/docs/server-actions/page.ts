...
export default function NewPost({ actionData }: {
  ...
  \`;
    ...
    <p><code>formaction=\${fn}</code> on a submit button is refused rather than supported. A per-submitter identity would have to ride the submitter's own <code>name</code>/<code>value</code> pair, and that pair is how a multi-button form tells its buttons apart. A form with several buttons binds ONE action and dispatches on a button's <code>name</code>.</p>
