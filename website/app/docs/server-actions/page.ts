...
export default function NewPost({ actionData }: {
  ...
  \`;
    ...
    <p>The submitter must be a real submit control (<code>&lt;button&gt;</code> or <code>&lt;input type="submit"&gt;</code>) inside a form that is itself bound, and it cannot carry its own <code>name</code>, <code>value</code>, <code>form</code>, or a static <code>formaction</code>, because the identity already occupies that name/value pair. An <code>&lt;input type="image"&gt;</code> is refused too: it submits <code>name.x</code> / <code>name.y</code> coordinates rather than <code>name=value</code>, so the identity would never arrive. Separately, <code>formmethod="get"</code> and an unparseable <code>formenctype</code> like <code>text/plain</code> are refused on ANY submitter inside a bound form, binding or not, since neither can carry the action's body. <code>formmethod="dialog"</code> and a plain <code>formaction="/url"</code> are left alone, because neither submits to the bound action.</p>
