...
export async function createPost(input: CreatePostInput): Promise&lt;ActionResult&lt;Post&gt;&gt; {
  ...
  return { success: true, data: post };
    ...
    <p>The envelope is additive, so beyond the two fields above a failure may also carry <code>fieldErrors</code> (per-field messages), <code>values</code> (what the user typed, so a re-render can repopulate the form), and a success may carry <code>redirect</code> (a same-site local path). The framework acts on <code>fieldErrors</code> and <code>redirect</code> itself. <code>values</code> it simply hands back to the page on <code>ctx.actionData</code>, so repopulating the inputs stays the page author's job (<code>value=\${actionData.values?.field}</code>), which is what the validation example further down does.</p>
...
export default function NewPost({ actionData }: {
  ...
  \`;
    ...
    <p><strong>A bound submitter carries its whole submission, so the enclosing form does not have to be bound.</strong> The renderer puts <code>formmethod="post"</code> and <code>formenctype="multipart/form-data"</code> on the button itself, alongside the identity, which is what React does for a button carrying a function <code>formAction</code>. So a per-button action works inside a bound form, an unbound form, a <code>method="get"</code> form, or a form with no method at all. No <code>formaction</code> url is emitted, because an empty one is an HTML conformance error, so the submission targets whatever the form targets: a form declaring its own <code>action="/x"</code> sends its buttons there, still running the action, since the identity travels in the body.</p>
    <p>The submitter must be a <code>&lt;button&gt;</code>, and it cannot carry its own <code>name</code>, <code>value</code>, <code>form</code>, or a static <code>formaction</code>, because the identity already occupies that name/value pair. Two input controls are refused for the same underlying reason. <code>&lt;input type="image"&gt;</code> submits <code>name.x</code> / <code>name.y</code> coordinates rather than <code>name=value</code>, so the identity would never arrive. <code>&lt;input type="submit"&gt;</code> would receive the identity in its <code>value</code>, which on that control is also the visible caption, so it would render captioned with the action id and the only way to label it is the channel the identity needs. A <code>&lt;button&gt;</code> avoids both, because its label is its children. A <code>.prop</code> spelling of any of these (<code>.name</code>, <code>.value</code>, <code>.formAction</code>, <code>.formMethod</code>, <code>.formEnctype</code>) is refused as well: all reflect on a submitter, so the write is dropped at SSR and lands in the attribute in the browser. Separately, a BOUND submitter is refused when it declares a <code>formmethod</code> other than post, an unparseable <code>formenctype</code> like <code>text/plain</code>, or <code>formmethod="dialog"</code>: each contradicts the action attached to that same button. A button that binds NOTHING is left alone, whatever it declares. Its <code>formmethod</code> / <code>formenctype</code> is a legal native override, the author wrote it deliberately, and the form's action simply does not run, exactly as the same markup behaves anywhere else. In dev the client logs a console error at submit time when a submission is carrying an identity it cannot deliver.</p>
    ...
    <p>To make that same form feel instant without changing the action, see <a href="/docs/optimistic-ui">Optimistic UI</a>.</p>
  `;
}
