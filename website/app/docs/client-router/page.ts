...
export default function ClientRouter() {
  return html`
    ...
    <p><strong>Wire-byte optimization</strong>: the router sends an <code>X-Webjs-Have</code> request header listing <code>segment:route-key</code> entries for the boundaries it already has (the key lets the server re-render a dynamic layout the client holds for different params instead of skipping it). The server walks the target page's layout chain innermost-to-outermost, short-circuits at the first match, and returns only the divergent fragment wrapped in that layout's boundary pair. Outer layouts are never re-serialized for same-shell navigations, and a reduced response carries <code>Vary: X-Webjs-Have</code> so a shared cache can never serve it to a full-page navigation.</p>
...
&lt;a href="/dashboard" data-prefetch="none"&gt;never (alias: false)&lt;/a&gt;</pre>
    ...
    <pre>document.addEventListener('webjs:navigation-fallback', (e) =&gt; {
