...
&lt;meta name="view-transition" content="same-origin"&gt;</code-block>
    ...
    <p>A traversal that stays on the same path and query is the exception, and it restores nothing. Only the fragment can differ, so the document never changed and the browser has already done whatever the step needed. That covers a Back or Forward between two fragment states of one page, and a repeat click of one in-page anchor, which replaces its history entry rather than pushing a new one and still fires <code>popstate</code>, arriving with an unchanged URL but always with a <code>#</code>. The router absorbs both, so live DOM identity and hydrated component state survive them. A <em>fragmentless</em> step between two entries that share a URL is still a navigation, because that pair is real: a bound form emits no <code>action</code>, so its 422 re-render pushes a duplicate entry at the page's own URL, and Back from a validation error has to re-render.</p>
