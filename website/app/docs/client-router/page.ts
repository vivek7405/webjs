...
&lt;a href="/dashboard" data-prefetch="none"&gt;never (alias: false)&lt;/a&gt;</code-block>
    ...
    <p><strong>A route that streams does not get this.</strong> The server slices the subtree only when the render did not stream, so a route with a <code>loading.{js,ts}</code> or a <code>Suspense</code> boundary answers every framed request with the whole document instead. The router will not cache that under a frame key (it would swap an entire document into the frame region), so on those routes the click still costs its round trip, exactly as before. The refusal is remembered for the cache TTL, so the link re-asks at most once per TTL rather than on every hover.</p>
