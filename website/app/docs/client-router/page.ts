...
export default function ClientRouter() {
  return html`
    ...
    <ol>
      ...
      <li>Custom elements upgrade in two waves, which is worth knowing if you write a swapped-in script. An element whose definition the page ALREADY has upgrades as step 5 inserts it, synchronously, so a script running afterwards sees it constructed and connected. Then every <code>&lt;script&gt;</code> the swap brought in re-executes, and a final upgrade pass catches whatever is still not upgraded, including elements those very scripts just defined. Every <code>&lt;script&gt;</code> the swap brings in runs again, whether it sits inside the swapped content or is a top-level node of the swapped range itself (a layout emitting its enhancement script as a sibling of <code>\${children}</code>). A parsed script node carries the HTML spec's "already started" flag, so the router replaces it with a fresh clone; that is what makes it run. The clone carries the page-load CSP nonce, not the nonce the response was rendered with. The one exception is a script INSIDE an element the swap preserved by identity through <code>data-webjs-permanent</code>, covered below.</li>
