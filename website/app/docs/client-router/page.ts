...
export default async function PostPage({ params }) {
  ...
  \`;
    ...
    <p>One thing this rule does NOT cover, because the router never sees it: a <em>pure</em> fragment link, whose path and query match the page it sits on and only the hash differs. The click handler bows out before <code>preventDefault</code>, so the browser performs its own native fragment jump and the window does move. A back-to-top anchor inside a frame therefore behaves exactly as it does anywhere else on the page, which is what a reader expects of it.</p>
    <p>The escapes are page navigations and DO scroll to top: <code>data-webjs-frame="_top"</code>, and an id matching no live frame (which warns and degrades to a normal navigation). Turbo's <code>autoscroll</code> opt-in, which scrolls the frame itself into view on swap, has no WebJs equivalent; the router simply never writes scroll for a frame.</p>
