...
export default function ClientRouter() {
  return html`
    ...
    <ol>
      ...
      <li>The URL updates via <code>pushState</code>, and this happens <strong>before the incoming content replaces what is on screen</strong>, not after. The order is deliberate: WebKit binds a same-document history entry's back-forward gesture snapshot to the page state at the moment the entry is recorded, so recording it after the content swap makes an iOS back-swipe preview the destination page instead of the page being returned to. One exception worth knowing: if a <code>loading.&#123;js,ts&#125;</code> skeleton is showing (it is applied optimistically, before the response arrives), the entry is recorded against that skeleton rather than the page you left.</li>
...
export default async function PostPage({ params }) {
  ...
  \`;
    ...
    <p><strong>Read "never scrolls" as "WebJs never writes a scroll", not as a promise the viewport cannot move.</strong> A swap that changes the panel's height can still shift the offset: the browser clamps a scroll position the document is no longer tall enough to hold, and a swap removes content before inserting its replacement, so the document is briefly shorter than either end state. On the gallery's frames demo, filtering from All to Done while scrolled to the bottom moves the window, with the router writing nothing. It bites only near the bottom of a page whose frame shrinks, and keeping the frame a stable height across its states avoids it.</p>
