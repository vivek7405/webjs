...
export default function ClientRouter() {
  return html`
    ...
    <ol>
      ...
      <li>Scripts the swap brought in re-execute, and custom elements upgrade. You do not have to sequence those two against each other: an element whose definition the page already has upgrades as step 5 inserts it, synchronously, and <code>customElements.define()</code> upgrades any matching elements already in the page at the moment a swapped-in script registers one. So a script sees its own elements upgraded either way. The router keeps its own upgrade call and a document-wide observer as backstops for anything those two miss, such as a subtree a View Transition deferred. That re-execution reaches a script wherever it sits, inside the swapped content or as a top-level node of the swapped range itself (a layout emitting its enhancement script as a sibling of <code>\${children}</code>). A parsed script node carries the HTML spec's "already started" flag, so the router replaces it with a fresh clone; that is what makes it run. The clone carries the page-load CSP nonce, not the nonce the response was rendered with. The one exception is a script INSIDE an element the swap preserved by identity through <code>data-webjs-permanent</code>, covered below.</li>
