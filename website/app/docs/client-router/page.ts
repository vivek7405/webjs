...
export default async function PostPage({ params }) {
  ...
  \`;
    ...
    <p>A page navigation scrolls to top, the way a browser does. A frame swap does not: it changes one region and leaves the rest of the document standing, the reader's scroll offset included. Without that, filtering a panel below the fold would throw the reader back to the top of the page, with the panel they just clicked in off screen. The rule covers every way a frame swaps, a nested link, an external <code>data-webjs-frame</code> trigger, a frame-targeted form submission, and a <code>src</code> self-load, and it covers a <code>#hash</code> in a frame link too, since a frame swap is not a document navigation.</p>
