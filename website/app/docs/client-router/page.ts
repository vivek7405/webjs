...
&lt;meta name="view-transition" content="same-origin"&gt;</code-block>
    ...
    <p>A traversal that stays on the same path and query is the exception, and it restores nothing. Only the fragment can differ, so the document never changed and the browser has already done whatever the step needed. That covers a Back or Forward between two fragment states of one page, and it covers a repeat click of one in-page anchor, which replaces its history entry rather than pushing a new one and still fires <code>popstate</code>. The router absorbs both, so live DOM identity and hydrated component state survive them.</p>
