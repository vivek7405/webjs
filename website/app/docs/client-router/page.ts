...
&lt;a href="/dashboard" data-prefetch="none"&gt;never (alias: false)&lt;/a&gt;</pre>
    ...
    <h3>The prefetch cache is context-keyed, not just URL-keyed</h3>
    <p>A prefetched fragment is a <em>reduced</em> response. The request carries <code>X-Webjs-Have</code> (the layout boundaries the client already holds) and the server returns only the divergent part, marking the response <code>Vary: X-Webjs-Have</code>. An entry is therefore a fragment <em>relative to</em> the DOM it was fetched against, so the cache stores that context alongside it and checks it on consume. A mismatch discards the entry and the click refetches: one round-trip, versus handing the swap a fragment that shares no boundary with the live DOM, which would correctly degrade to a full page load.</p>
