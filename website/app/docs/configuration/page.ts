...
&#125;</code-block>
    ...
    <p>The browser logs one warning per page view when a hydration call missed its seed, naming which cause applies, and stays silent when every call hit. Development also warns once per action function when a single render returns two different results for the same arguments, the one case where a seed can disagree with the paint the user is looking at. None of this reaches production: no header, no marker, and prod HTML is byte-identical.</p>
