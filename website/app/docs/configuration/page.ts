...
&#125;</code-block>
    ...
    <p><strong>What it catches, and what it does not.</strong> It reports an unknown top-level key, a value outside an allowed set (<code>trailingSlash</code>), and a wrong-typed boolean or whole number (<code>elide</code>, <code>seed</code>, <code>clientRouter</code>, <code>maxBodyBytes</code>, the timeouts). It does not descend into nested objects, so a key misspelled inside <code>webjs.dev</code> or <code>webjs.start</code> is missed, and it does not check the shape of the array, string, and object-valued keys, so a wrong-shaped <code>headers</code>, <code>redirects</code>, <code>allowedOrigins</code>, <code>basePath</code>, <code>csp</code>, <code>dev</code>, <code>start</code>, or <code>doctor</code> passes it and is dropped by its reader as before. Those are the free-form shapes, where a blunt check would start rejecting configs that work. Editors catch more of this earlier: a scaffolded app wires the schema into <code>.vscode/settings.json</code>, and the <code>WebjsConfig</code> type from <code>@webjsdev/core</code> types the block while you author it.</p>
