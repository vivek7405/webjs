...
&#125;</code-block>
    ...
    <p>Every key in the <code>webjs</code> block is optional, which means an unknown one has no way to announce itself: write <code>"redirect"</code> for <code>"redirects"</code> and the key is simply never read, the feature sits at its default, and the app looks configured. So WebJs validates the block against its published JSON Schema once per boot, in development and in production alike, and prints a single warning naming every unknown key and every value of the wrong type or outside its allowed set.</p>
    <p>It is a <strong>warning, never a failure</strong>. A typo costs one feature its setting, and refusing to boot over that would cost the whole app, usually mid-deploy. The check is also top-level only, so a key misspelled inside a nested object (<code>webjs.dev</code>, <code>webjs.start</code>, <code>webjs.csp</code>) is still missed. Editors catch the same class earlier: a scaffolded app wires the schema into <code>.vscode/settings.json</code>, and the <code>WebjsConfig</code> type from <code>@webjsdev/core</code> types the block while you author it.</p>
