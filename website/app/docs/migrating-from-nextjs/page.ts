  ...
  return html`
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>next/link</code></td><td>A plain <code>&lt;a href&gt;</code>. The client router auto-enhances same-origin links into partial-swap navigations. Prefetch is on by default; tune it with <code>data-prefetch</code>. <code>&lt;Link scroll={false}&gt;</code> is <code>data-preserve-scroll</code>, which also works on a wrapping element and on a form.</td></tr>
...
Counter.register('my-counter');</code-block>
    ...
    <p><strong>Needs rethinking:</strong> anything written as a Client Component becomes a web component; anything fetching server data moves into a <code>.server</code> action; React state becomes signals; <code>next/link</code> becomes a plain link. Write progressive-enhancement-first: a <code>&lt;form&gt;</code> plus a server action instead of a <code>fetch</code> in a click handler, since the form works without JavaScript and the client router upgrades it automatically. The form binding ports directly: <code>&lt;form action={serverAction}&gt;</code> in Next is <code>&lt;form action=\${serverAction}&gt;</code> here, and per-button submitter actions work via <code>&lt;button formaction=\${deleteAction}&gt;</code>. What differs is underneath: React serializes the binding, bound arguments included, into hidden fields, while WebJs emits ONE hidden field per bound action carrying its identity and no arguments, so a per-row action takes its row id from a hidden input rather than from <code>action.bind(null, id)</code>.</p>
