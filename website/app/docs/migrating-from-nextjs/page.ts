...
Counter.register('my-counter');</pre>
    ...
    <p><strong>Needs rethinking:</strong> anything written as a Client Component becomes a web component; anything fetching server data moves into a <code>.server</code> action; React state becomes signals; <code>next/link</code> becomes a plain link. Write progressive-enhancement-first: a <code>&lt;form&gt;</code> plus a server action instead of a <code>fetch</code> in a click handler, since the form works without JavaScript and the client router upgrades it automatically. Note the binding differs from Next's: <code>&lt;form action={serverAction}&gt;</code> has no WebJs equivalent and is refused at render (it would leak the action's source into the HTML), so omit <code>action</code> to post to the page's own URL and handle the submission in that page's <code>action</code> export.</p>
