...
export default function Security() {
  return html`
    ...
    <ul>
      ...
      <li>CSRF protection on server-action calls, over RPC and over a form submission alike (Origin / Sec-Fetch-Site check, no token cookie).</li>
    ...
    <h2>CSRF: an Origin / Sec-Fetch-Site check</h2>
    <p>Server-action calls are CSRF-protected by a cross-origin check, the model Remix 3 and Go 1.25 use. It covers BOTH ways an action is invoked: an RPC call through the typed stub generated from a <code>'use server'</code> import, and a form submission bound with <code>&lt;form action=\${fn}&gt;</code>. On a state-changing request the server reads the <code>Sec-Fetch-Site</code> fetch-metadata header (browser-set, unforgeable by page JS): <code>same-origin</code> and <code>none</code> pass. When that header is absent (an older browser), it falls back to comparing the <code>Origin</code> host against the request host. A cross-site request is rejected with 403 unless its origin is listed in <code>webjs.allowedOrigins</code>. You write nothing; importing the action is the whole API, whether you call it or bind it to a form, and there is no token or cookie to manage. A no-JS form therefore needs no hidden CSRF token field.</p>
    <p>Because there is no per-request CSRF cookie, SSR responses carry no <code>Set-Cookie</code>, so a page that opts into a public <code>Cache-Control</code> (via <code>metadata.cacheControl</code>, e.g. on a root layout for a whole visitor-identical app) can be cached at a CDN edge. One caveat when you do: a CDN keys its copy on its own request, not on <code>X-Forwarded-Host</code>, so a page that derives absolute URLs from <code>ctx.url</code> should not be made publicly cacheable, or pin that origin from config instead. For a reverse-proxy or multi-domain setup, list the extra origins the check should accept in <code>webjs.allowedOrigins</code> (the check honors <code>x-forwarded-host</code> unless <code>WEBJS_NO_TRUST_PROXY=1</code> tells it nothing trusted sits in front of the container).</p>
