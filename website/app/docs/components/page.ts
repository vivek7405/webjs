...
UserCard.register('user-card');</code-block>
    ...
    <p>Property names are automatically converted between camelCase (JavaScript) and kebab-case (HTML). A property named <code>userName</code> observes the attribute <code>user-name</code>. HTML attribute names are case-insensitive and the browser lowercases them while parsing, so write the attribute in kebab-case: a camelCase attribute in markup (<code>userName="…"</code>) reaches no property, in the browser or on the server.</p>
