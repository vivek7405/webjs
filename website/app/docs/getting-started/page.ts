...
webjs create my-api --template api</pre>
    ...
    <p><strong>Naming the app.</strong> The name you pass becomes three things at once: the new directory, the <code>name</code> field of the generated <code>package.json</code>, and a value the scaffold writes into generated source (the home page's <code>metadata.title</code>, for one). So it has to be a valid npm package name: lowercase letters, digits, and the separators <code>-</code>, <code>.</code> and <code>_</code>, starting with a letter or a digit, at most 214 characters, and not <code>node_modules</code> or <code>favicon.ico</code>, which npm reserves. Anything else is refused before a single file is written, with a message naming the offending character. <code>webjs create MyApp</code> and <code>webjs create "my app"</code> are both errors, so write <code>webjs create my-app</code>.</p>
