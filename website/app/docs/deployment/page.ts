...
&#125;</pre>
    ...
    <p><strong>The process exits <code>0</code> in one case only: an operator asked for the stop AND it drained cleanly.</strong> Everything else is a <code>1</code>. A drain that fails is a failure on its own terms, so a server close that errors, or a drain still hanging when the 10-second deadline fires, exits <code>1</code> whichever signal started it. When the drain succeeds, the code instead reports why the process is going down: <code>SIGINT</code> or <code>SIGTERM</code> exits <code>0</code>, while a shutdown started by an uncaught exception exits <code>1</code> even though the drain was perfectly clean, because the process is dying from an error. Your supervisor reads only that code, so this is what makes <code>Restart=on-failure</code> (systemd), a Docker restart policy, and a Railway or Kubernetes crash-loop detector actually fire on a crashed server instead of recording a successful exit and leaving the instance down.</p>
