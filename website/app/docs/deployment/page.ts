...
npm run start -- --port 8080</pre>
    ...
    <p><strong>Unstorable content is excluded.</strong> A page with the default <code>Cache-Control: no-store</code> (every dynamic / per-user page) gets <em>no</em> ETag and never returns a 304: <code>no-store</code> forbids storage outright, so there is nothing to validate. A <code>private</code> response IS validated, because <code>private</code> forbids only SHARED storage and the ETag hashes that response's own body, so one user's validator can never match another's. <strong>Streaming responses are excluded too</strong>, both streamed Suspense pages and a <code>route.&#123;js,ts&#125;</code> handler that returns a <code>ReadableStream</code> (including an SSE <code>text/event-stream</code>). Their bodies are never buffered to be hashed, so a long-lived or never-ending stream is never read into memory and never stalls the response. A 304 preserves the validators and caching headers (<code>ETag</code>, <code>Cache-Control</code>, <code>Vary</code>) and drops only the body.</p>
