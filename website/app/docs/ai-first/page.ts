    ...
    <code-block>// .cursor/mcp.json (or your host's MCP config)
{
  "mcpServers": {
    ...
    <p>The docs corpus is resolved from the app being asked about, not from wherever the server happens to be installed: the copy in that app's <code>node_modules/@webjsdev/mcp</code> wins over the snapshot the server was published with, then the server's own snapshot, then the live repo-root docs in a monorepo checkout. It matters for a globally installed server, which otherwise serves the docs it shipped with forever. <code>init</code> reports which corpus it served on its first line, and warns when the app's <code>@webjsdev/mcp</code> is newer than the running server's, so a stale global install is visible instead of silently teaching reverted guidance.</p>
