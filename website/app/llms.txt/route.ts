...
export async function GET(): Promise<Response> {
  ...
  const lines: string[] = [
    ...
    '',
    'WebJs is inspired by Next.js, Lit, and Rails, but ships its own no-build runtime: TypeScript is stripped at load, ES modules are served directly, and the view layer is web components rather than React. It is designed to be read end to end by AI coding agents, works with any assistant through one cross-agent contract, and scaffolds a production-shaped app from the first command.',
    '',
    'Key facts:',
    '- Agent-agnostic, not tied to one vendor: a single cross-agent `AGENTS.md` contract plus a skill drives Claude, Cursor, Copilot, Gemini, and others. The component API mirrors Lit, so existing model knowledge transfers with no framework-specific training.',
