...
const FEATURES = [
  ...
  { icon: '🧱', title: 'Web Components First', desc: 'Less is more: only hooks with no native workaround (render, setState, firstUpdated, renderError). Reactive controllers for composition. Context protocol for shared state. Shadow DOM + Declarative Shadow DOM for real SSR.' },
  ...
  { icon: '🛡️', title: 'Batteries Included', desc: 'Sessions, background jobs, file storage, pub/sub, cache, rate limiting — all with pluggable adapters. Set REDIS_URL and everything scales horizontally. Convention over configuration, like Rails.' },
