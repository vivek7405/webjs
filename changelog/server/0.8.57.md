...
- **derive the request origin safely, and honor `X-Forwarded-Proto` / `X-Forwarded-Host` on Bun** ([#1091](https://github.com/webjsdev/webjs/pull/1091)) [`8ce50392`](https://github.com/webjsdev/webjs/commit/8ce50392)
  ...
  **This affects both runtimes. Upgrade if your app derives any absolute URL.**
  ...
  The origin was built by resolving the request target against the proxy headers, and four inputs a client controls could poison it. The first two apply to every runtime, the third and fourth to node and Bun respectively:
