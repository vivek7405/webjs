...
export class WsEcho extends WebComponent {
  ...
  private applyStreamUpdate() {
    this.#streamN += 1;
    renderStream(
      `<webjs-stream action="append" target="ws-stream-log"><template><li class="px-3 py-2 rounded-xl bg-card border border-border text-sm text-foreground">streamed row #${this.#streamN}</li></template></webjs-stream>`,
    );
  }
      ...
      <div class="grid gap-4 max-w-md">
        ...
        <ul class="grid gap-1.5 list-none m-0 p-0">
          ${this.lines.get().map((line) => html`
            <li class="px-3 py-2 rounded-xl bg-card border border-border text-sm text-foreground font-mono">${line}</li>
          `)}
        </ul>
