...
export function WS(ws: WebSocket, req: Request, { params }: { params: Record&lt;string, string&gt; }) {
  ...
  ws.on('close', () =&gt; clients.delete(ws));
    ...
    <p>Use the <code>json()</code> helper from <code>@webjs/server</code> and the <code>richFetch()</code> client helper from <code>webjs</code> for superjson-encoded responses that preserve <code>Date</code>, <code>Map</code>, <code>Set</code>, and <code>BigInt</code>:</p>
...
// curl http://localhost:3000/api/events</pre>
    ...
    <p>For reading request bodies with the same content negotiation, use <code>readBody(req)</code> from <code>@webjs/server</code>.</p>
...
fastify.listen({ port: 3000 });</pre>
  ...
  return resp;
    ...
    <pre>import { json } from '@webjs/server';
import { prisma } from '../../../lib/prisma.ts';
