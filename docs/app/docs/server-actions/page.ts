...
PostForm.register('post-form');</pre>
    ...
    <ul>
      ...
      <li><code>RegExp</code>, <code>Error</code> -- preserved</li>
      <li>Nested combinations of all the above</li>
    </ul>
    <p>The stub serialises function arguments with <code>superjson.stringify(args)</code> and deserialises the response with <code>superjson.parse(text)</code>. The server does the inverse. This is invisible to the developer -- you work with native types on both sides.</p>
...
export const createPost = expose('POST /api/posts', async ({ title, body }: { title: string; body: string }) =&gt; {
  return prisma.post.create({ data: { title, body } });
    ...
    <ul>
      <li><strong>From a client component:</strong> <code>import { createPost } from '../actions/posts.server.ts'</code> -- auto-rewritten to RPC stub, CSRF-protected, superjson-encoded.</li>
...
export class TodoApp extends WebComponent {
  ...
  async connectedCallback() {
    ...
    this.setState({ todos });
    // todos[0].createdAt is a real Date (superjson preserved it)
  }
