...
5. Re-run the scripts the swap brought in, then make a final custom-element upgrade pass. The pass is a safety net rather than the first upgrade: an element whose definition the page already holds upgrades as a custom-element reaction the instant the swap inserts it, so what is left for the pass is mostly an element one of those just-re-executed scripts has only now defined.
