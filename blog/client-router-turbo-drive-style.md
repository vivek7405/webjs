...
5. Re-run the scripts the swap brought in, upgrading custom elements alongside. The upgrade calls are backstops rather than the first upgrade, because the platform already handles both ordinary cases: an element whose definition the page holds upgrades as a custom-element reaction the instant the swap inserts it, and `customElements.define` upgrades matching elements already in the page, so an element a just-re-executed script defines is upgraded by that call rather than by the router's. What is left for the backstops is a subtree the insertion reactions never reached, such as one a View Transition deferred.
