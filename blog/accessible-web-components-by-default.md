...
The rest follow the same discipline. The dropdown menu declares `aria-haspopup="menu"`, uses `role="menu"` and `role="menuitem"` with roving focus and `aria-disabled`. The tooltip wires `aria-describedby` from the trigger to the tip text. The toaster (`<ui-sonner>`) is a polite live region, so new toasts get announced, and an error toast additionally carries `role="alert"` so it is announced assertively. An ordinary toast carries no role of its own, which keeps it under a single live region: nesting a second one inside the viewport bought nothing and made some screen readers announce the toast twice. Across the set, interactive elements carry `focus-visible` ring styles so keyboard users can see where focus is.
