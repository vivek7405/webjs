...
12. **A form that writes binds its action: `<form action=${importedAction}>`** (#1155). That is the only shape the renderer reads, and every near-miss throws rather than producing a form that posts nowhere: a quoted `action="${fn}"`, a `formaction=${fn}` on a submit button (write one form per action), `action=${fn}` on any tag other than `<form>`, a bound form declaring `method="get"`, and a function that is not a `'use server'` export. A page has no `action` export, so a bare `<form method="post">` is a `405`.
