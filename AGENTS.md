...
The `<name>` must be a valid npm package name (lowercase letters, digits, and the separators `-`, `.`, `_`, starting with a letter or a digit, at most 214 characters). It becomes the app directory, the generated `package.json` `name`, AND a value the scaffold writes into generated source, so anything else is rejected before a single file is written (#1066). `webjs create MyApp` is an error; write `webjs create my-app`.
