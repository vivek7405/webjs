...
4. **Convention validation.** Run `webjs check` and fix violations. Run `webjs doctor` too when you touched an in-repo app (`examples/blog`, `website`): the required `conventions` CI job runs it over both, and it fails on a hard toolchain check or on whatever that app's `webjs.doctor.gate` marks `error` (today `UNMARKED_ASSET_LINKS` in `website` and `examples/blog`), so a clean `webjs check` alone is not enough to predict that job (#1257).
...
12. **A form that writes binds its action: `<form action=${importedAction}>`, and a form whose buttons run different actions binds each on its submitter, `<button formaction=${importedAction}>` inside that bound form** (#1155, #1207). Those are the two shapes the renderer reads, and every near-miss throws rather than producing a form that posts nowhere. The submitter identity rides the pressed button's own `name`/`value` pair (the one channel a browser submits for that button alone), so no `formaction` url is emitted, both entries reach the server, and the dispatcher takes the LAST `__webjs_action` in DOM order, which is the submitter's whenever one was pressed. **Refused on a form:** a quoted `action="${fn}"`, `action=${fn}` on any tag other than `<form>`, `method="get"` or an enctype the server cannot parse, a `.method` / `.enctype` / `.encoding` PROPERTY binding (a `.prop` drops at SSR and applies in the browser, so the form would submit differently with JS than without), a second `action` hole, a plain `action="/url"` alongside the bound hole (SSR keeps it and the client drops it), a whitespace-padded `method=" post "` (an enumerated attribute is matched against exact keywords, so a padded value falls to the invalid-value default and submits as a GET), and a function that is not a `'use server'` export. **Refused on a submitter:** an enclosing form that is not itself bound (`method` and the enctype are forced on the form's start tag, too late to add from the button), a control that is not a submit control, an `<input type="image">` (it submits `name.x` / `name.y` coordinates, so the identity never arrives), an `<input type="submit">` (the identity has to occupy its `value`, which on that control is also its visible label, so it would render captioned with the action id and could never be labelled; a `<button>` has no such conflict), a submitter carrying its own `name`, `value`, `form`, or static `formaction`, a `.prop` spelling of any of those (`.name` / `.value` / `.formAction` / `.formMethod` / `.formEnctype` all REFLECT on a submitter, so the write is dropped at SSR and lands in the attribute in the browser), and a second `formaction` hole. **Refused on ANY submitter inside a bound form, whether or not it binds:** `formmethod="get"` and an unparseable `formenctype` such as `text/plain`, since either works under JS (the router posts `FormData`) and is a bare `405` without it. Two carve-outs there: `formmethod="dialog"` never submits, and a plain `formaction="/url"` retargets away from the bound action. WebJs supplies `method` and `enctype` only where your template supplies neither, judged from the TEMPLATE rather than the rendered element: `?method=${false}` emits nothing so it is supplied, while `method=${null}` emits `method=""` and is refused. An `encoding=` attribute is inert in HTML (only the `.encoding` PROPERTY aliases `enctype`), so both renderers ignore it. A page has no `action` export, so a bare `<form method="post">` is a `405`. **Boundness is BEST EFFORT in both renderers, and neither always knows.** SSR reads a linear byte stream, so it knows within one template scan, but a COMPONENT renders its own template in a separate pass with no view of the host page, and the client may reconcile a submitter whose form lives in a parent template and is not in the tree yet (a `repeat()` row). Each renderer therefore distinguishes three answers: bound (fine), unbound (refused), and cannot-tell (bind anyway). Cannot-tell has to bind: refusing there would reject a per-row button in a list or a button inside a component, both ordinary shapes, and at SSR a refused component is ISOLATED, so in production the button would silently vanish from a page that still returned 200. The refusal still fires wherever the answer is genuinely known. **`webjs check`'s `submitter-needs-bound-form` rule (#1307) closes the residual statically**, because it has neither renderer's limit: it reads every template in the app at once, so it flags a submitter whose enclosing form is unbound in the same scan AND one whose component is only ever rendered inside an unbound form, across module boundaries and transitively through intermediate components. It is deliberately conservative and stays silent on anything it cannot resolve (a tag with mixed call sites, a tag with no call site, an unbound call site that still DELIVERS the identity, a dynamic `method` / `enctype` hole, a submitter in a bare `html` helper, a multi-component file, a file that opens a form of its own, a submitter or tag handed to another element through a start-tag hole, a `formaction` hole that is not a proven action binding (a url string, a url CONSTANT exported from a `.server` module, a factory-produced export, a namespace or default import, a barrel re-export, or any non-identifier expression such as `acts.publishDraft` or `this.act`), a reference cycle), so it can never false-positive on the ordinary shapes the cannot-tell fallback exists to protect. The one start-tag hole it DOES judge is `<webjs-suspense .fallback=${…}>`, because the renderer renders a fallback inline in the enclosing form rather than handing it off. Worth knowing what the residual actually does when it ships, because it is narrower than it first looks. An unbound form that still sends a parseable POST body WORKS: the submitter's identity rides its own `name`/`value` pair, so it reaches the dispatcher and the action runs. What breaks is an unbound form that sends nothing readable. With no `method` (or `method="get"`) the browser submits a GET, so the identity rides the QUERY STRING, the action never runs, and the page re-renders with a 200 and nothing thrown; with an unparseable enctype it is a bare 405. The rule flags only the cross-module cases that cannot deliver, and flags a SAME-scan unbound form whatever its method, because the renderer refuses that shape outright. Two more layers make that loud instead of silent. In DEV the client logs one `console.error` at submit time when the submission cannot carry the identity (it logs and never throws, because the listener is delegated at the document and a throw there would abort before `preventDefault` and make dev behave differently from production). In PRODUCTION both server-visible fingerprints reach the `onError` hook with a code: `WEBJS_FORM_SUBMITTED_AS_GET` for a page GET carrying the field in its query string, and `WEBJS_FORM_ACTION_MISSING` for a parseable form body carrying no identity (an `enctype="text/plain"` submission is answered by the non-form 405 before its body is read, so that variant stays a bare 405 and only the dev client guard flags it). Both are detect-only (the GET keeps its 200, the bind-nothing submission keeps its 405), carry the submitted field NAMES but never the values, and are deduplicated per process on the code, the method, and the matched ROUTE (not the request pathname, so crafted urls on a dynamic route cannot exhaust the 256-entry cap and silence the diagnostics), because either fingerprint is reachable by an unauthenticated attacker and an uncapped report would be a free amplifier into a paid APM sink.
