...
**Design guidance is a reference, not a checker.** `references/design.md` carries what a screen should look like before the class list is written (hierarchy, the action pyramid, spacing rhythm, type scale, palette, elevation, empty states) and `references/design-depth.md` carries palette construction, the per-family variant checklist, and the finishing pass. Read the core one alongside `references/styling.md` on any user-facing screen. `@webjsdev/ui` backs it with **semantic role tokens** (`--success` / `--warning` / `--info` / `--destructive`, each with a foreground and a subtle surface pair) and a **role-named elevation scale** (`shadow-e1` raised through `shadow-e4` top; in dark the lift comes from the surface being lighter than the page, which `--card` and `--popover` already express, and the shadow only separates the edge). State goes through a role, never a palette family: a hardcoded `text-emerald-500` is what makes an app un-rethemable. None of this is in `webjs check`, which carries correctness rules only; taste stays the agent's job.
