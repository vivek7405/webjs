...
4. `npm run check` must pass (correctness), and so must `npm run doctor`
   (project health). CI runs both, and doctor fails on whatever your
   `package.json` `webjs.doctor.gate` marks `error`, which starts as the
   un-versioned stylesheet link check. Everything else it reports is a warning
   that cannot fail the build, so widen or narrow the gate in `package.json`
