...
|---|---|---|
| `components/` | **NO: gitignored**, regenerated by `webjs.dev.before` / `webjs.start.before` (`scripts/copy-registry.js`, #550) from `../registry/`. | ❌ **NEVER.** Anything you put here is wiped every dev cycle AND never reaches the deploy. |
...
`components/ui/*.ts` (run via `webjs.dev.before` / `webjs.start.before`, #550),
rewriting each component's internal
...
pages have something to import. Re-running `npm run dev` (or `webjs dev`)
re-populates from the current registry state.
