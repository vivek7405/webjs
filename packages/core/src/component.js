...
/**
 * Typed component factory — the recommended entry for new components
 * written in TypeScript. See `component.d.ts` for the inference rules.
 *
 * Returns a `WebComponent` subclass with `static properties` pre-set to
 * the passed descriptor map. At runtime this is a tiny subclass; the
 * heavy lifting is done by the TypeScript overlay that maps the property
 * descriptors to instance-field types, so authors get full autocomplete
 * and type-checking without a `declare` boilerplate line.
 *
 *     // TS
 ...
 *     }
 *
 * Plain-JS consumers still get the runtime benefit (fewer `static properties`
 * boilerplate chars); they just won't see the typed fields in their editor.
 *
 * @param {Record<string, any>} properties
 */
export function defineComponent(properties) {
  class Defined extends WebComponent {}
  Defined.properties = properties || {};
  return Defined;
}

/**
 * Type-only helper — identity at runtime. Exists so TypeScript can carry
 * a caller-supplied value type through the property descriptor.
 *
