...
| `action-seed-client.js` | Client consumer for SSR action-result seeding (#472): `takeSeed(hash, fn, argsKey)` (consume-once lookup, `SEED_MISS` sentinel) + `scanSeeds(root)` (ingest the page-level `#__webjs-seeds` JSON block and per-element `[data-webjs-seed]` carriers, stripping them). The generated RPC stub calls `takeSeed` before its `fetch`, so an async-render component's first client call resolves from the SSR seed instead of a hydration round-trip; `router-client.js`'s `applySwap` calls `scanSeeds` so a soft nav ingests seeds too. Ingest is LAST-write-wins (#1309): a hit DELETES its key, so a duplicate can only mean a later render seeded a key an earlier one left unconsumed, and the later render is the one on screen. That rule is only safe because of the ORDERING `scanSeeds` enforces: a DETACHED root (what `applySwap` passes) means a new page is arriving, so the LIVE document is drained first. Without it the outgoing page's block, which a page whose async components all elided leaves behind (the initial scan is lazy) and which no swap removes (it sits outside every boundary range), is ingested LAST by the lazy scan and wins the keys both renders share, so a hit contradicts the paint. A live root or live subtree is not a new page and never triggers the drain. Dev observability (#1309): `seedStats()` returns `{ ingested, replaced, hits, misses, pending }`, and one `console.warn` fires at the first idle after a scan when a call missed, gated by the SERVER-stamped `data-webjs-dev` marker on the block rather than `process.env.NODE_ENV` (esbuild folds that to a constant here and `publicEnvShim` then inverts the guard). Each scan CLOSES the previous report window and opens its own, so a window measures exactly the page it was opened for; a markerless swap (a `<webjs-frame>` self-load, whose subtree `ssr.js` returns before the seed block is appended) falls back to the drained live page's marker rather than reporting nothing at all. Inert server-side (DOM access only inside `scanSeeds`). Re-exported from `index.js` + `index-browser.js` |
