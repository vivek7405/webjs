...
2. **Judge each finding MINOR or MUST-FIX** (rule below), then fix, reject, or defer it (the three dispositions below). **Only a FIX buys another round**, because only a fix changed the branch there is something new to read. A rejection buys one REFUTER instead, a deferral buys nothing, and minor findings are applied and buy nothing.
...
5. **Judge the final review the same way, then STOP.** Its fixes get ONE delta-scoped check of those fix commits alone; its rejections get a refuter; its deferrals and minor findings are recorded and applied. The cycle ends when that check finds nothing must-fix, and immediately when there was nothing to fix. A check that does find something must-fix gets that fixed and ONE more check of the same shape, and if that one does too, stop and report the PR unfinished. The final review is never re-run and the delta rounds are never re-entered.
