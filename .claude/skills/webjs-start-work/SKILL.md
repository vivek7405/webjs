...
**Only a round in which fresh subagents actually REVIEWED counts toward `<K>`, and a deep-review run, all its lenses and juries together, is ONE round.** A spawn that was declined, errored, was killed, or returned nothing did not produce a round, so it can be neither the clean last round nor part of the total, and an inline pass you did yourself is not a round at all (see the liveness rules in step 1). Neither does a spawn that returned a result without reviewing: a completed call and a real subagent result are NOT the test, because a reviewer that says "I could not fetch the diff" satisfies both and reviewed nothing. The test is that it came back with findings or with the literal `CLEAN`. A loop whose reviewer never reviewed has run ZERO rounds, and the honest report there is that the review is blocked and why, not a count and not "ready to merge".
