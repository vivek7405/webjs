...
2. **Judge each finding MINOR or MUST-FIX** (rule below), then fix, reject, or defer it (the three dispositions below). **Only a FIX buys another round**, because a fix is the only disposition that changes the branch, and a round with nothing new to read just re-poses the same question. A rejection buys one REFUTER instead, a deferral buys nothing, and minor findings are applied and buy nothing.
3. **Each later round is delta-scoped: ONE fresh reviewer whose QUESTION is the previous round's fix commits.** Repeat until a round produces no fixes, whether because it found nothing must-fix or because everything it found was rejected or deferred.
4. **The first round that produces no fixes buys the FINAL review: ONE fresh reviewer over the WHOLE diff again**, on the head the fixes produced. Two different jobs are being done here. After delta rounds, it is the first read of the finished change rather than of a fragment. After a clean round 1, when it is the same diff at the same head, it is a second independent read, which is the check a single-reviewer cycle owes in place of the fleet's second opinion. Either way a clean round is not the end of the cycle. This is.
5. **Judge the final review the same way, then STOP.** Its fixes get ONE delta-scoped check of those fix commits alone; its rejections get a refuter; its deferrals and minor findings are recorded and applied. The cycle ends when nothing must-fix is left OPEN, which means the check came back with nothing, or there was no fix to check because every must-fix finding it raised was refuted or deferred. A check that does find something must-fix gets that fixed and ONE more check of the same shape, and if that one does too, stop and report the PR unfinished. The final review is never re-run and the delta rounds are never re-entered.

A clean round 1 goes straight to the final review, so every PR gets at least two reviews. That is a deliberate reversal of the old one-round minimum, which was written when round 1 could be a 16-agent fleet that had already refuted its own findings.
...
**Keep the cycle fast.** After a fix, run only the test file(s) covering the line you changed, with the counterfactual toggle the Definition of done mandates (a fix can make an older test non-discriminating without failing it). The e2e, full Node, browser, and Bun suites and the four-app dogfood check run ONCE, after the cycle ends. Never wait on CI between rounds; read it once at the end. Both rules change WHEN work happens, never WHETHER.

**Do not restore what this replaced.** This cycle used to run a 16-agent fleet with a scout, parallel lenses, and a jury, pick round 1's shape by a path check, sort findings into two tiers, cap itself at five rounds, and poll a file to watch each spawn. All of it is gone on purpose: termination is structural now (only a fix buys a round, delta rounds narrow the question, the minor call stops wording from buying rounds, the final review plus one fix-check is a hard end), and reviews are async, so the harness notification plus one short check-in replace the polling. The one thing that came BACK is a two-review minimum, because a lone reviewer's miss is the cost of dropping the fleet and the final pass is what covers it.

Each round must:

1. **Spawn the round's reviewer** per the spawn spec, and act on the harness completion notification. Do not poll it and do not re-read its message hoping for a different answer.
   ...
   **Liveness.** Only two things are evidence a reviewer is alive: the harness status, and byte growth on its transcript when the output file is one. Its own prose never is (one signed off with "I'm partway through the careful pass" while its status read `killed`).
   ...
   - **The check-in must NEVER block your turn.** Run it as a BACKGROUND command, never a foreground `sleep`. A foreground wait re-creates the synchronous block that spawning the reviewer in the background exists to avoid: the turn stalls, the user watches a running shell command instead of progress, and you cannot do anything useful with the minutes the reviewer is working. If there is nothing else to do while it runs, do NOTHING and let the completion notification arrive; that is strictly better than a blocking probe. This is a real regression that happened while writing this rule, which is why it is stated first.
   - **Check in about every 2 minutes, and NEVER kill on the timer.** The timer is a wake-up, not a cap: a reviewer that hangs at one minute should cost about two, and a long cap just bounds the waste (a ten-minute one would not even have fired for the 8.6-minute reviewer measured on this repo). What decides is GROWTH, never elapsed time. Each check-in, `stat -c %s` the spawn's output file, following the symlink to its TARGET: if the byte count is rising it is working, so leave it alone however long it has run, and re-spawn only when it is FLAT across two consecutive check-ins or the harness reports killed or errored. Killing a reviewer that is still writing throws away its work and buys a full restart, which costs more than the wait ever did. Flatness is a signal ONLY where the file is a live transcript. Where it is a static stub instead, it is flat from the start no matter how healthy the reviewer is, so never re-spawn a stub-backed one on flatness: the harness notification is the only news, and the only backstop is a generous elapsed cap of about 15 minutes, comfortably past the roughly 9-minute reviews measured on this repo. Never read the transcript's contents: it is large enough to swamp your own context, and a subagent's words are not evidence either way.
