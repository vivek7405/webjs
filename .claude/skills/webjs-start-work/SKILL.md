...
2. **Judge each finding MINOR or MUST-FIX** (rule below). Must-fix findings are fixed, rejected, or deferred per step 2 and buy another round. Minor findings are applied and buy nothing.
3. **Each later round is delta-scoped: ONE fresh reviewer whose QUESTION is the previous round's fix commits** (or, when a round produced no fix commits, the riskiest section of the original diff). Repeat until a round surfaces nothing must-fix.
4. **The first round with nothing must-fix buys the FINAL review: ONE fresh reviewer over the WHOLE diff again**, on the head those fixes produced. The delta rounds only ever asked about fragments, so until now nobody has read the finished change end to end. A clean delta round is not the end of the cycle. This is.
5. **Judge the final review the same way.** Minor findings are applied and the cycle ends. Must-fix findings are fixed, then ONE delta-scoped reviewer checks those fix commits alone and the cycle ends on its result, without re-running the final review. If that check is also must-fix, fix and run one more of the same shape; still breaking after two is signal about the change, so stop and report it unfinished.

A clean round 1 skips step 3 and goes straight to the final review, so the minimum is two reviews.
...
1. **Spawn the round's reviewer** per the spawn spec, and act on the harness completion notification. Do not poll it and do not re-read its message hoping for a different answer.
   ...
   **Liveness.** Only two things are evidence a reviewer is alive: the harness status, and byte growth on its transcript when the output file is one. Its own prose never is (one signed off with "I'm partway through the careful pass" while its status read `killed`).
   ...
   - **Check in on a SHORT timer, and never auto-kill on it.** Set it at about 2 to 3 minutes, not ten: a cap only bounds the waste, so a reviewer that hangs at one minute costs nine wasted ones under a ten-minute cap. Make it a wake-up, never a kill, because killing throws away a healthy reviewer still reading a large diff and buys a full restart. On each check-in, look at the spawn's output file: it may be a symlink to a live transcript, in which case a growing byte count means it is working and you leave it alone (`stat -c %s` on the symlink TARGET, twice, seconds apart), or it may be a static stub, in which case there is no mid-flight signal and the wait is blind but bounded. Never read the transcript's contents; it is large enough to swamp your own context, and a subagent's words are not evidence either way. A silent one gets a second short window, then counts as hung and is re-spawned. The completion notification arrives independently, so a reviewer that finishes first makes the timer moot.
...
2. **For each finding, do exactly ONE of three things.** There is no fourth, and "mention it and move on" is not one. One gate first: a must-fix finding on shipped source whose fix would be expensive or behavior-changing gets an adversarial REFUTER first, a fresh spawn told to DISPROVE the claim (does it reproduce in the code as written, is the behavior intended, is it already guarded or tested somewhere the finder did not look). A refuted finding is a rejection carrying the refuter's reason; if no refuter can be produced, act on the finding ungated. Trivial or obviously-real findings skip the gate.
