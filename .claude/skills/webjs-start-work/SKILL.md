...
3. **Each later round is delta-scoped: ONE fresh reviewer whose QUESTION is the previous round's fix commits.** Repeat until a round produces no fixes, whether because it found nothing must-fix or because everything it found was rejected or deferred. **A delta chain that keeps producing fixes stops after the FIFTH**, unfinished, reported per the reporting section rather than continued: a change whose repairs keep breaking something is signal about the change, not about the review, and the measured 18-round loop that motivated this spent its last four rounds catching regressions its own fixes had introduced.
