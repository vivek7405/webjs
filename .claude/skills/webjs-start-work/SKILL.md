...
1. **Spawn the round's reviewer** per the spawn spec, and act on the harness completion notification. Do not poll it and do not re-read its message hoping for a different answer.
   ...
   **Liveness.** Only two things are evidence a reviewer is alive: the harness status, and byte growth on its transcript when the output file is one. Its own prose never is (one signed off with "I'm partway through the careful pass" while its status read `killed`).
   ...
   - **A progress check is optional, runs in the BACKGROUND, and never kills.** Elapsed time is not evidence: reviewers here routinely run 5 to 10 minutes while working normally. The only positive signal is the transcript growing (`stat -c %s` on the spawn's output path, following the symlink); rising bytes mean it is working, so leave it alone however long it has run. Nothing is a reliable stall signal, since a file that never grows may simply be a stub the harness does not write to, so never re-spawn on a flat file or on a timer. Re-spawn on a killed or errored harness status, or when YOU judge the wait unreasonable and are willing to pay for the restart. Never read the transcript's contents: it is large enough to swamp your own context, and a subagent's words are not evidence either way.
