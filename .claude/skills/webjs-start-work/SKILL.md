...
```
Review PR #<N> (branch `<branch>`) at https://github.com/webjsdev/webjs/pull/<N> for bugs, regressions, security issues, missed edge cases, broken invariants, doc drift, test gaps, and style violations against the project's AGENTS.md and CONVENTIONS.md (root + per-package). <For a fan-out reviewer, replace this broad charter with its single narrow lens. For a delta round, scope every step to the fix commits' diff and the paragraphs they touched instead of the whole PR; for a pass following a round with no fix commits (a clean fan-out, or all findings rejected), scope to the riskiest section of the original diff.>
