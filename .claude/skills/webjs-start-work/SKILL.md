...
- If a delta chain reaches its fifth round still producing fixes: stop there, withhold the flip to ready for review, and report the rounds run, every finding still open, and your read on why each fix keeps buying another round. The branch, the commits, and the card all stay exactly as they are, and the deferred suites do not run, since they gate a flip that is not happening.
