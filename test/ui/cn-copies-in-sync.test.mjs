 ...
 *
 *   packages/ui/packages/registry/lib/utils.ts   the canonical one, which
 *                                                `webjs create` copies verbatim
 ...
 *
 * TWO is the whole inventory of hand-synced SOURCES. Other copies of this file
 * exist on disk and are GENERATED, so they need no guard and must not be edited:
 * `website/lib/utils/cn.ts` is written by `website/scripts/copy-registry.mjs`
 * from the registry (gitignored via `website/.gitignore`, regenerated by the
 * `pretest` hook and by `webjs.dev.before` / `webjs.start.before`), and a
 * scaffolded app's `lib/utils/cn.ts` is written by `webjs create` itself,
 * through `writeUiBootstrap()` in `packages/cli/lib/create.js`, which reads
 * this file off disk. A stale generated copy on someone's disk is not drift;
 * re-running its generator is the fix.
 *
 * `webjs ui init` is a DIFFERENT path and not a copy of this file: it is the
 * non-scaffold flow (`packages/ui/src/commands/init.js`) and fetches `lib-utils`
 * over HTTP from the PUBLISHED registry, so it tracks the last release rather
 * than the working tree, and a fix here reaches it only once `@webjsdev/ui`
 * ships.
 */
