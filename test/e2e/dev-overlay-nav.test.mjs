...
describe('E2E: dev error overlay URL scope (#1047)', {
  ...
  test('a back/forward restore does not resurrect the overlay', async () => {
    // Continues from /good, with /flaky one step back in history and its
    // snapshot taken while its overlay was live. Heal the page first, so the
    // only thing that could put an overlay back on /flaky is the snapshot
    // having the old one BAKED IN: the router serializes
    // `document.documentElement.outerHTML`, so an overlay still in the DOM at
    // snapshot time comes back as a dead card whose Dismiss button has no
    // listener. That is what the `webjs:before-cache` handling prevents.
