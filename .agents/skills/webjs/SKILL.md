...
- Putting a `formaction=${fn}` submitter in a COMPONENT and forgetting to bind the `<form>` in the page that renders it. This is the one near-miss that does NOT throw: the component renders in a separate pass with no view of the host page, so the renderer binds anyway, and an unbound form submits as a GET with the identity in the query string. The action never runs, the page re-renders, the status is 200. Run `webjs check` (`submitter-needs-bound-form`) and see `muscle-memory-gotchas.md`.
