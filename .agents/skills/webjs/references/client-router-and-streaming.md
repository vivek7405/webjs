...
- **Do not write your own scroll restore.** A `popstate` listener that calls `scrollTo`, a saved offset in `sessionStorage`, a `scrollIntoView` on a remembered element: all of them fight the router, which restores from its own snapshot and is the effective authority on scroll during a navigation. If Back lands in the wrong place, that is a framework bug to report, not something to patch in app code.
- **The router does NOT set `history.scrollRestoration = 'manual'`, and an app must not either** (#1428). Under the default `auto` the browser records a scroll position per history entry, and WebKit composes the iOS edge back-swipe GESTURE PREVIEW from that recorded state. Taking `manual` suppresses the recording, so every scrolled page previews BLANK for the whole gesture, which is what the router used to do (inherited from Turbo Drive's `assumeControlOfScrollRestoration`, and Turbo still previews blank the same way). The reason `manual` was tempting is real: the browser's own restore lands a frame AFTER the popstate handler and would otherwise overwrite the router's. That is handled inside the restore window instead, which writes back an off-target programmatic displacement for its span, so the recording survives and the race does not. An app that sets `manual` re-breaks the preview app-wide.
