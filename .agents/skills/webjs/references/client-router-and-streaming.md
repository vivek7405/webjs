...
**Read "never moves" as "WebJs never writes one", not as a guarantee the viewport cannot move.** A swap that changes the panel's HEIGHT can still shift the offset, because the browser clamps a scroll position the document is no longer tall enough to hold, and a swap replaces content before it inserts the replacement, so the document is briefly shorter than either end state. Measured on the gallery's own frames demo: filtering from All to Done while scrolled to the bottom moves the window even though the router wrote nothing (verified with `window.scrollTo`, `scrollIntoView` and the `scrollTop` setter all instrumented, and it is NOT scroll anchoring, since forcing `overflow-anchor: none` changes nothing). It bites only near the bottom of a page whose frame shrinks. Keeping the frame's height stable across its states avoids it entirely.
