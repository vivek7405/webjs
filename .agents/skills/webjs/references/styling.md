...
**Next.js solves this in the framework, and WebJs currently does not. Know the difference, because it is the one place a Next habit is safe there and unsafe here.** Next renders each segment's CSS INSIDE that segment's tree (`getLayerAssets` runs per layout or page), as `<link rel="stylesheet" precedence>` or, for inlined CSS, `<style precedence href>`. The `precedence` attribute is React's resource-hoisting contract: React lifts the element out of the tree into `<head>` and dedupes it by `href`. In production nothing ever removes it again, and the client's runtime-style set is add-only. So a Next author colocates CSS with the route and the renderer keeps it out of the swapped tree for them.

WebJs has no equivalent. A `<style>` a page or layout renders stays exactly where it was rendered, inside the boundary, and churns. So the discipline Next automates is discipline you apply by hand: put the static rules in the compiled stylesheet. Automating it is tracked in #1112.

(Next's dev-only stylesheet GC additionally delays a superseded sheet's removal behind a `setTimeout`, commented "Delay the removal of the stylesheet to avoid FOUC". Useful independent evidence that swapping sheets mid-navigation flashes, but it is a development-mode cleanup, not the production strategy.)
