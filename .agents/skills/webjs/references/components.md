...
Most of what follows restates widely held component-model advice, ported. Lit's base class exists to hold reactive state, scoped styles, and a declarative template TOGETHER (the `lit` package README), and Lit documents a ref's value as `undefined` once the node "is no longer rendered", which is precisely the signal a selector lookup cannot give you. React frames the same ideas as lifting state to the closest common owner (react.dev, "Sharing State Between Components") and treating a ref as an escape hatch rather than the normal way to reach a node (react.dev, "Escape Hatches"). What WebJs changes is where the boundary falls, because two facts about its execution model have no counterpart in either framework. A page or layout NEVER hydrates, so its markup cannot carry interactivity: an `@click` in a page template is dropped at SSR, and the module's only client presence is its top-level side effects. And the client router's swap range never covers `<body>` or `<html>`, so the document shell outlives every navigation. Rules 3 and 5 both rest on that second fact, and rule 4's first caveat on a third WebJs specific, the way the two renderers disagree about a nullish or `false` attribute hole.
