...
- A FULLY STATIC `<style>` with no `${}` survives hydration, and a page or layout never hydrates at all, so neither is flagged by that rule. **Neither is free, and the reason is navigation, not hydration (#1109).** A `<style>` rendered by a component, a page, or a NON-ROOT layout is inside the client router's swap boundary, so crossing routes removes a stylesheet and inserts another, and a CSSOM mutation invalidates style document-wide (a preserved fixed header repaints). A component's also duplicates once per instance on the page. Put STATIC rules in the compiled stylesheet the ROOT layout links, tag-prefixed per the rule above; use `static styles` with `static shadow = true` for genuinely scoped CSS, which is adopted per element and never touches the document CSSOM. See `styling.md`.
